...
def _silent_qt_handler(mode, context, message):
    pass


qInstallMessageHandler(_silent_qt_handler)

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
TASKS_TXT = os.path.join(BASE_DIR, 'tasks.txt')
...
class ToDoList(QWidget):
    ...
    def save_tasks(self):
        ...
        with open(TASKS_TXT, 'w', encoding='utf-8') as file:
            file.write('\n'.join(lines))

    def load_tasks(self):
        if not os.path.exists(TASKS_TXT):
            return
        with open(TASKS_TXT, 'r', encoding='utf-8') as file:
            for raw in file:
                line = raw.rstrip('\n')
                if not line.strip():
                    continue
